...
class BaseLaplace:
    ...
    def __init__(self, model, likelihood, sigma_noise=1., prior_precision=1.,
        ...
        if likelihood not in ['classification', 'regression', 'language_modelling']:
...
class ParametricLaplace(BaseLaplace):
    ...
    def fit(self, train_loader, override=True):
        ...
        #if isinstance(out, (CausalLMOutputWithCrossAttentions, CausalLMOutput)):
        #    out = out.logits
        ...
        for data in train_loader:
            ...
            #H_batch_clipped = H_batch.narrow(0, 0, self.H.size(0))
            # or using slicing
            #H_batch_clipped = H_batch.narrow(0, 0, min(self.H.size(0), H_batch.size(0)))

            #self.H += H_batch_clipped
            #self.H += H_batch
            
            # print(f"in fit: H_batch type: {type(H_batch)}")
            
            if H_batch.size(0) > self.H.size(0):
                H_batch_clipped = H_batch.narrow(0, 0, self.H.size(0))
            elif H_batch.size(0) < self.H.size(0):
                # Pad H_batch with zeros to match the size of self.H
                padding = torch.zeros(self.H.size(0) - H_batch.size(0), device=H_batch.device)
                H_batch_clipped = torch.cat((H_batch, padding), dim=0)
            else:
                H_batch_clipped = H_batch
